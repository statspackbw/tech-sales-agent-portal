...
async function viewCompanies() {
    ...
    <div class="table-scroll"><table>
      ...
      <tbody>${d.companies.map(c => `<tr>
        ...
        <td><div class="btn-row">
          <button class="btn ghost sm" data-co="${c.id}">Edit</button>
          <button class="btn ghost sm" data-coteam="${c.id}" data-coname="${esc(c.name)}">People</button>
          ${c.is_host ? '' : `<button class="btn danger sm" data-codel="${c.id}" data-coname2="${esc(c.name)}">Delete</button>`}
        </div></td>
  ...
  document.querySelectorAll('[data-coteam]').forEach(b => b.onclick =
    () => companyPeopleModal(Number(b.dataset.coteam), b.dataset.coname));
  document.querySelectorAll('[data-codel]').forEach(b => b.onclick =
    () => deleteCompanyModal(Number(b.dataset.codel), b.dataset.coname2));
}

async function companyPeopleModal(companyId, companyName) {
  let people;
  try {
    people = (await api(`/api/admin/companies/${companyId}/users`)).users;
  } catch (e) { return reportError(e); }

  modal('People at ' + companyName, `
    <p class="muted" style="margin-bottom:14px">Resetting a password signs that person out
      immediately and issues a one-time password for you to pass on. They choose their own at
      next sign-in.</p>
    ${people.length ? `<div class="table-scroll"><table>
      <thead><tr><th>Name</th><th>Role</th><th>Status</th><th></th></tr></thead>
      <tbody>${people.map(u => `<tr>
        <td><strong>${esc(u.name)}</strong><div class="muted" style="font-size:12px">${esc(u.email)}</div></td>
        <td>${u.role === 'admin' ? 'Administrator' : esc(u.agent_type || 'Agent')}</td>
        <td><span class="badge ${u.status}">${u.status}</span></td>
        <td>${u.is_super ? '<span class="muted" style="font-size:12px">super user</span>'
          : `<button class="btn ghost sm" data-rp="${u.id}" data-rpname="${esc(u.name)}">Reset password</button>`}</td>
      </tr>`).join('')}</tbody></table></div>`
      : '<p class="muted">Nobody is registered at this company yet.</p>'}`,
    `<button class="btn ghost" onclick="closeModal()">Close</button>`);

  document.querySelectorAll('[data-rp]').forEach(b => b.onclick = async () => {
    if (!confirm(`Reset the password for ${b.dataset.rpname}? They will be signed out now.`)) return;
    const done = busy(b, 'Resetting…');
    try {
      const r = await api(`/api/admin/companies/${companyId}/users/${b.dataset.rp}/reset-password`,
                          'POST', {});
      modal('New temporary password', `
        <p>Pass these to ${esc(r.name)}. The password is shown once.</p>
        <div class="copybox">${esc(r.email)}<br>${esc(r.temp_password)}</div>
        <p class="hint">They must choose their own password at first sign-in.</p>`,
        `<button class="btn teal" onclick="closeModal()">Done</button>`);
    } catch (e) { reportError(e); done(); }
  });
}

async function deleteCompanyModal(companyId, companyName) {
  let info;
  try {
    info = await api(`/api/admin/companies/${companyId}/footprint`);
  } catch (e) { return reportError(e); }
  const f = info.footprint;

  modal('Permanently delete ' + companyName, `
    <div class="danger" style="margin-bottom:0">
      <h3>This deletes an entire organisation</h3>
      <p>Every account, client and payment belonging to ${esc(companyName)} is erased. Their
        administrator and agents lose access immediately. There is no undo, and a backup taken
        afterwards will not contain any of it.</p>
    </div>

    <p style="margin:16px 0 8px"><strong>About to be destroyed:</strong></p>
    <div class="danger-list"><ul>
      <li><strong>${f.users}</strong> account(s) — ${f.admins} administrator(s), ${f.agents} agent(s)${
        f.user_names.length ? '<div class="muted" style="font-size:12.5px;margin-top:2px">'
          + f.user_names.map(esc).join(', ') + (f.users > f.user_names.length ? ', …' : '') + '</div>' : ''}</li>
      <li><strong>${f.clients}</strong> client(s)</li>
      <li><strong>${f.payments}</strong> payment(s), totalling <strong>${usd(f.collected_usd)}</strong> collected</li>
      <li><strong>${f.payouts_paid}</strong> payout(s) already marked paid, worth <strong>${usd(f.payouts_paid_usd)}</strong></li>
      <li><strong>${f.collaborations}</strong> partnership record(s)</li>
    </ul></div>

    <p class="muted" style="margin-bottom:14px">If you only want to stop them using the portal,
      <strong>suspend</strong> the company instead — that signs everyone out at once and keeps
      every record.</p>

    <label class="lbl" for="dqName">Type <strong>${esc(companyName)}</strong> to confirm</label>
    <input class="inp confirm-input" id="dqName" placeholder="${esc(companyName)}" autocomplete="off">`,
    `<button class="btn ghost" onclick="closeModal()">Cancel</button>
     <button class="btn ghost" id="dqExport">Download their records</button>
     <button class="btn danger" id="dqGo" disabled>Delete permanently</button>`);

  const nameInput = $('dqName'), go = $('dqGo');
  nameInput.oninput = () => {
    go.disabled = nameInput.value.trim().toLowerCase() !== companyName.trim().toLowerCase();
  };
  $('dqExport').onclick = async () => {
    try {
      const data = await api(`/api/admin/companies/${companyId}/export`);
      const a = document.createElement('a');
      a.href = URL.createObjectURL(new Blob([JSON.stringify(data, null, 2)], {type: 'application/json'}));
      a.download = 'statspack-company-' + companyName.replace(/[^a-z0-9]+/gi, '-').toLowerCase()
                   + '-' + new Date().toISOString().slice(0, 10) + '.json';
      a.click();
      URL.revokeObjectURL(a.href);
      toast('Company records downloaded.', 'ok');
    } catch (e) { reportError(e); }
  };
  go.onclick = async () => {
    const done = busy(go, 'Deleting…');
    try {
      const r = await api(`/api/admin/companies/${companyId}/purge`, 'DELETE',
                          {confirm_name: nameInput.value.trim()});
      closeModal();
      toast(`${companyName} deleted — ${r.deleted.users} account(s), `
            + `${r.deleted.clients} client(s) removed.`, 'ok');
      viewCompanies();
    } catch (e) { reportError(e); done(); }
  };
}
