...
@route("GET", r"/api/admin/companies/(\d+)/users")
def company_users(ctx, company_id):
    """Who can sign in for this company, so a super user can reset a password."""
    need_super(ctx)
    company_id = int(company_id)
    if not ctx.conn.one("SELECT id FROM companies WHERE id = ?", (company_id,)):
        raise ApiError(404, "Company not found.")
    rows = ctx.conn.query(
        "SELECT * FROM users WHERE company_id = ? ORDER BY role, name", (company_id,))
    return {"users": [public_user(u, full_avatar=False) for u in rows]}


@route("POST", r"/api/admin/companies/(\d+)/users/(\d+)/reset-password")
def reset_company_user_password(ctx, company_id, user_id):
    """Issue a new one-time password for someone at a client company."""
    need_super(ctx)
    company_id, user_id = int(company_id), int(user_id)
    user = ctx.conn.one("SELECT * FROM users WHERE id = ? AND company_id = ?",
                        (user_id, company_id))
    if not user:
        raise ApiError(404, "That person is not in this company.")
    if user.get("is_super") and user["id"] != ctx.user["id"]:
        raise ApiError(403, "You cannot reset another super user's password.")

    pw = core.temp_password()
    ctx.conn.execute("UPDATE users SET password_hash = ?, must_change_pw = 1 WHERE id = ?",
                     (core.hash_password(pw), user_id))
    core.purge_sessions(ctx.conn, user_id)
    core.audit(ctx.conn, ctx.user, "company.password_reset",
               f"{user['name']} <{user['email']}>", ctx.ip, target=user["email"],
               meta={"company_id": company_id, "user_id": user_id, "role": user["role"]})
    return {"temp_password": pw, "email": user["email"], "name": user["name"]}


@route("GET", r"/api/admin/companies/(\d+)/footprint")
def company_footprint_ep(ctx, company_id):
    need_super(ctx)
    company_id = int(company_id)
    ...
    return {"company": {"id": company["id"], "name": company["name"],
                        "is_host": bool(company.get("is_host"))},
            "footprint": core.company_footprint(ctx.conn, company_id),
            "can_delete": not bool(company.get("is_host"))}


@route("GET", r"/api/admin/companies/(\d+)/export")
def company_export_ep(ctx, company_id):
    need_super(ctx)
    data = core.export_company(ctx.conn, int(company_id))
    if not data:
        raise ApiError(404, "Company not found.")
    core.audit(ctx.conn, ctx.user, "company.exported", data["company"]["name"], ctx.ip)
    return data


@route("DELETE", r"/api/admin/companies/(\d+)/purge")
def purge_company_ep(ctx, company_id):
    """Permanently delete a client company and everything belonging to it."""
    need_super(ctx)
    company_id = int(company_id)
    company = ctx.conn.one("SELECT * FROM companies WHERE id = ?", (company_id,))
    if not company:
        raise ApiError(404, "Company not found.")
    if company.get("is_host"):
        raise ApiError(400, "The host company cannot be deleted — it is your own "
                            "organisation and holds your super-user account.")
    if (ctx.user.get("company_id") or 1) == company_id:
        raise ApiError(400, "You cannot delete the company your own account belongs to.")

    typed = ctx.s("confirm_name")
    if typed.strip().lower() != company["name"].strip().lower():
        raise ApiError(400, "Type the company name exactly to confirm.")

    footprint = core.company_footprint(ctx.conn, company_id)
    counts = core.purge_company(ctx.conn, company_id)
    core.audit(ctx.conn, ctx.user, "company.PURGED",
               f"{company['name']} — removed {counts['users']} user(s), "
               f"{counts['clients']} client(s), {counts['payments']} payment(s); "
               f"${footprint['collected_usd']:,.2f} of collections erased",
               ctx.ip, target=company["name"],
               meta={"company_id": company_id, **counts,
                     "collected_usd": footprint["collected_usd"]})
