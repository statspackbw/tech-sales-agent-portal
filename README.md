# StatsPack Tech Sales Agent Portal — v1.6
...
### Resetting a password for a client company

Companies page → **People**. You see everyone who can sign in for that company,
administrators and agents alike, and can reset any of their passwords. The reset
signs them out immediately and produces a one-time password to pass on; they
must choose their own at next sign-in.

Super users only. A company administrator cannot use this route, so they cannot
reset each other's access. You can reset your own super-user password here too.

### Deleting a company

Companies page → **Delete**. This removes the whole organisation: every account,
client, payment, payout, review, activity entry and partnership belonging to it.
Their people lose access immediately.

Before you confirm you are shown exactly what will go — how many accounts and
whose, how many clients, how many payments and how much money they represent —
and offered a full JSON download. You must type the company name.

Blocked by design: the host company (your own organisation, holding your
super-user account), and any company your own account belongs to.

**Suspend is usually the better answer.** It signs everyone out at once and
keeps every record. The dialog says so.

Verified in `test_v16.py`: after deleting a company, its admin and agents can no
longer sign in, the host company's money and people are untouched, and nothing
is orphaned — no stray payments, timeline entries, payouts, reviews,
partnerships, users or clients pointing at records that no longer exist.
...
559 checks across the fifteen files, covering authentication, role separation, the
...
| `test_v16.py` | Company deletion, password reset, tenant isolation on delete |
