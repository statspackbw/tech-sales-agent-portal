...
with sync_playwright() as p:
    ...
    pg.goto(B+"/#/companies", wait_until="networkidle"); pg.wait_for_timeout(1000)
    ok("People button present", pg.query_selector("[data-coteam]") is not None, "missing")
    pg.query_selector("[data-coteam]").click(); pg.wait_for_timeout(1400)
    h = pg.query_selector(".modal-head h3")
    ok("people dialog opens", h and "People at" in h.inner_text(), h.inner_text() if h else "none")
    ok("reset button offered", pg.query_selector("[data-rp]") is not None
       or "Nobody is registered" in pg.inner_text(".modal-body"), pg.inner_text(".modal-body")[:70])
    pg.keyboard.press("Escape"); pg.wait_for_timeout(400)
    pg.goto(B+"/#/companies", wait_until="networkidle"); pg.wait_for_timeout(900)
    delbtn = pg.query_selector("[data-codel]")
    if delbtn:
        delbtn.click(); pg.wait_for_timeout(1400)
        h = pg.query_selector(".modal-head h3")
        ok("company delete dialog opens", h and "Permanently delete" in h.inner_text(),
           h.inner_text() if h else "none")
        ok("confirm field disabled until typed", pg.query_selector("#dqGo").is_disabled(), "enabled")
        pg.keyboard.press("Escape")
    else:
        ok("no delete button on the host company (correct)", True)
