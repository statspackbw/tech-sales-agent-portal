...
def bootstrap(conn):
    ...
    for code, rate, label in DEFAULT_FX:
        if not conn.one("SELECT code FROM fx WHERE code = ?", (code,)):
            conn.execute(
                "INSERT INTO fx (code, rate, label, updated_at) VALUES (?,?,?,?)",
                (code, rate, label, now_iso()),
            )
    ...
    conn.insert(
        ...
        "must_change_pw, quota_usd, start_date, notes, created_at) "
        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
        ("StatsPack Admin", admin_email, "", "Lesotho", "admin", "Active",
         hash_password(admin_pw), 1 if generated else 0, 0, today(), "", now_iso()),
    )
