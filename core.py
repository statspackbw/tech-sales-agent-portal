...
def active_collaborations(conn, client_ids):
    """Every partnership that has ever been live on these clients, keyed by
    client. Ended ones are included deliberately: commission is attributed by
    when a payment arrived, not by whether the partnership is still running.
    """
    if not client_ids:
        return {}
    marks = ",".join("?" for _ in client_ids)
    rows = conn.query(
        f"SELECT * FROM collaborations WHERE status IN ('Accepted','Ended') "
        f"AND client_id IN ({marks}) ORDER BY responded_at, id",
        tuple(client_ids))
    out = {}
    for r in rows:
...
def collab_for_payment(collabs, client_id, payment):
    """The partnership that was running when this payment was recorded, if any.

    Two decisions worth stating plainly:

    A partnership that has ENDED still earns its partner a share of everything
    recorded while it was live. Removing that retroactively would claw back
    money the partner had already been paid and hand it to the lead.

    The test is when the payment was RECORDED, not the date on the invoice.
    Payments are often entered late or backdated; what matters is whether the
    two agents were working the client together when the money was processed.
    """
    stamp = str(payment.get("created_at") or payment.get("paid_date") or "")[:19]
    if not stamp:
        return None
    for k in collabs.get(client_id, []):
        started = str(k.get("responded_at") or "")[:19]
        if not started or stamp < started:
            continue                       # recorded before the partnership began
        ended = str(k.get("ended_at") or "")[:19]
        if ended and stamp > ended:
            continue                       # recorded after it finished
        return k
    return None
...
def split_payments(payments, collabs):
    ...
    """
    for p in payments:
        collab = collab_for_payment(collabs, p.get("client_id"), p)
...
def agent_commission_summary(conn, agent_id, settings):
    ...
    partnered = conn.query(
        "SELECT client_id FROM collaborations WHERE partner_id = ? "
        "AND status IN ('Accepted','Ended')", (agent_id,))
...
def agent_footprint(conn, agent_id):
    ...
    collabs = conn.one("SELECT COUNT(*) AS n FROM collaborations "
                       "WHERE (owner_id = ? OR partner_id = ?) AND status = 'Accepted'",
                       (agent_id, agent_id))
    return {
        "collaborations": int(collabs["n"] or 0),
        "clients": len(clients),
...
def export_agent(conn, agent_id):
    ...
    return {
        ...
        "collaborations": conn.query(
            "SELECT * FROM collaborations WHERE owner_id = ? OR partner_id = ?",
            (agent_id, agent_id)),
...
def purge_agent(conn, agent_id):
    ...
    counts = {"payments": 0, "clients": len(ids), "payouts": 0, "reviews": 0,
              "sessions": 0, "timeline": 0, "collaborations": 0}
    if ids:
        marks = ",".join("?" for _ in ids)
        ...
        conn.execute(f"DELETE FROM collaborations WHERE client_id IN ({marks})", tuple(ids))
    row = conn.one("SELECT COUNT(*) AS n FROM collaborations "
                   "WHERE owner_id = ? OR partner_id = ?", (agent_id, agent_id))
    counts["collaborations"] = counts["collaborations"] + int(row["n"] or 0)
    ...
    conn.execute("DELETE FROM collaborations WHERE owner_id = ? OR partner_id = ?",
                 (agent_id, agent_id))
...
def company_footprint(conn, company_id):
    """Everything that would be destroyed with a company."""
    users = conn.query("SELECT id, name, email, role, status FROM users WHERE company_id = ?",
                       (company_id,))
    clients = conn.query("SELECT id, name FROM clients WHERE company_id = ?", (company_id,))
    ids = [c["id"] for c in clients]
    payments = collected = 0
    if ids:
        marks = ",".join("?" for _ in ids)
        row = conn.one(f"SELECT COUNT(*) AS n, COALESCE(SUM(amount_usd),0) AS t "
                       f"FROM payments WHERE client_id IN ({marks}) AND voided = 0", tuple(ids))
        payments = int(row["n"] or 0)
        collected = round(float(row["t"] or 0), 2)
    agent_ids = [u["id"] for u in users] or [0]
    marks = ",".join("?" for _ in agent_ids)
    payouts = conn.one(f"SELECT COUNT(*) AS n, COALESCE(SUM(amount_usd),0) AS t "
                       f"FROM payouts WHERE agent_id IN ({marks}) AND status = 'Paid'",
                       tuple(agent_ids))
    collabs = conn.one("SELECT COUNT(*) AS n FROM collaborations WHERE company_id = ?",
                       (company_id,))
    return {
        "users": len(users),
        "admins": len([u for u in users if u["role"] == "admin"]),
        "agents": len([u for u in users if u["role"] == "agent"]),
        "user_names": [u["name"] for u in users][:25],
        "clients": len(clients),
        "client_names": [c["name"] for c in clients][:25],
        "payments": payments,
        "collected_usd": collected,
...
def export_company(conn, company_id):
    settings = get_settings(conn)
    company = conn.one("SELECT * FROM companies WHERE id = ?", (company_id,))
    if not company:
        return None
    users = conn.query("SELECT * FROM users WHERE company_id = ?", (company_id,))
    for u in users:
        u.pop("password_hash", None)
    clients = conn.query("SELECT * FROM clients WHERE company_id = ?", (company_id,))
    ids = [c["id"] for c in clients]
    agent_ids = [u["id"] for u in users] or [0]
    marks = ",".join("?" for _ in agent_ids)
    return {
        "exported_at": now_iso(),
        "reason": "snapshot taken immediately before deleting a company",
        "company": company,
        "users": users,
        "clients": clients,
        "payments": client_payment_rows(conn, ids, settings, newest_first=False),
        "timeline": [e for cid in ids for e in client_timeline(conn, cid)],
        "payouts": conn.query(f"SELECT * FROM payouts WHERE agent_id IN ({marks})",
                              tuple(agent_ids)),
        "reviews": conn.query(f"SELECT * FROM reviews WHERE agent_id IN ({marks})",
                              tuple(agent_ids)),
        "collaborations": conn.query("SELECT * FROM collaborations WHERE company_id = ?",
                                     (company_id,)),
    }


def purge_company(conn, company_id):
    """Delete a company and everything belonging to it, in dependency order."""
    users = conn.query("SELECT id FROM users WHERE company_id = ?", (company_id,))
    clients = conn.query("SELECT id FROM clients WHERE company_id = ?", (company_id,))
    uids = [u["id"] for u in users] or [0]
    cids = [c["id"] for c in clients]
    counts = {"users": len(users), "clients": len(cids), "payments": 0,
              "timeline": 0, "payouts": 0, "reviews": 0, "collaborations": 0, "sessions": 0}

    if cids:
        marks = ",".join("?" for _ in cids)
        counts["payments"] = int((conn.one(
            f"SELECT COUNT(*) AS n FROM payments WHERE client_id IN ({marks})",
            tuple(cids)) or {}).get("n") or 0)
        counts["timeline"] = int((conn.one(
            f"SELECT COUNT(*) AS n FROM client_events WHERE client_id IN ({marks})",
            tuple(cids)) or {}).get("n") or 0)
        conn.execute(f"DELETE FROM payments WHERE client_id IN ({marks})", tuple(cids))
        conn.execute(f"DELETE FROM client_events WHERE client_id IN ({marks})", tuple(cids))
        conn.execute(f"DELETE FROM collaborations WHERE client_id IN ({marks})", tuple(cids))

    umarks = ",".join("?" for _ in uids)
    counts["payouts"] = int((conn.one(
        f"SELECT COUNT(*) AS n FROM payouts WHERE agent_id IN ({umarks})", tuple(uids))
        or {}).get("n") or 0)
    counts["reviews"] = int((conn.one(
        f"SELECT COUNT(*) AS n FROM reviews WHERE agent_id IN ({umarks})", tuple(uids))
        or {}).get("n") or 0)
    counts["sessions"] = int((conn.one(
        f"SELECT COUNT(*) AS n FROM sessions WHERE user_id IN ({umarks})", tuple(uids))
        or {}).get("n") or 0)
    counts["collaborations"] = int((conn.one(
        "SELECT COUNT(*) AS n FROM collaborations WHERE company_id = ?", (company_id,))
        or {}).get("n") or 0)

    conn.execute(f"DELETE FROM payouts WHERE agent_id IN ({umarks})", tuple(uids))
    conn.execute(f"DELETE FROM reviews WHERE agent_id IN ({umarks})", tuple(uids))
    conn.execute(f"DELETE FROM sessions WHERE user_id IN ({umarks})", tuple(uids))
    conn.execute(f"DELETE FROM collaborations WHERE owner_id IN ({umarks}) "
                 f"OR partner_id IN ({umarks})", tuple(uids) + tuple(uids))
    conn.execute("DELETE FROM collaborations WHERE company_id = ?", (company_id,))
    conn.execute("DELETE FROM clients WHERE company_id = ?", (company_id,))
